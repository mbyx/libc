...
{%- let c_field_name = self.c_ident(MapInput::Field(structure, field)).unwrap() +%}
...
{%- let volatile = self.volatile(VolatileItemKind::StructField(structure.clone(), field.clone())) +%}
...
{%- if self::should_roundtrip(generator, ident) +%}
{%- let c_type = self.c_type(*structure)? +%}
