    ...
    for entry in src.read_dir().unwrap() {
        ...
        if entry.file_type().unwrap().is_dir() {
            ...
            if !target.contains("linux") && !target.contains("apple") {
                ...
                std::fs::write(&dst_path, &src_data).unwrap();
            }
        }
    }
}
...
fn test_linux(target: &str) {
    ...
    test_linux_like_apis_next(target, cfg);
}
...
fn test_linux_like_apis_next(target: &str, cfg: ctest_next::TestGenerator) {
    let gnu = target.contains("gnu");
    let musl = target.contains("musl") || target.contains("ohos");
    ...
    if linux || android || emscripten {
        ...
        ctest_next::generate_test(
            &mut cfg,
            src_hotfix_dir().join("lib.rs"),
            "linux_strerror_r.rs",
        )
        .unwrap();
    }
    ...
    if linux || android || emscripten {
        ...
        ctest_next::generate_test(&mut cfg, src_hotfix_dir().join("lib.rs"), "linux_fcntl.rs")
            .unwrap();
    }
    ...
    if linux || android {
        ...
        ctest_next::generate_test(&mut cfg, src_hotfix_dir().join("lib.rs"), "linux_ipv6.rs")
            .unwrap();
    }
    ...
    if (linux && !wali) || android {
        ...
        ctest_next::generate_test(&mut cfg, src_hotfix_dir().join("lib.rs"), "linux_elf.rs")
            .unwrap();
    }
    ...
    if (linux && !wali) || android {
        ...
        ctest_next::generate_test(&mut cfg, src_hotfix_dir().join("lib.rs"), "linux_if_arp.rs")
            .unwrap();
    }
}
